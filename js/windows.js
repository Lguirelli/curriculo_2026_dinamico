...
function createBrowserFrameHTML(title, url, fallbackUrl=''){
  return `<section class="project-browser-content"><iframe class="project-browser-frame" src="${url}" title="${escapeHTML(title)}" data-fallback-url="${fallbackUrl}"></iframe></section>`;
}
...
function openHtmlApp(item){
  ...
  const win = createWindow({title:item.label, html, kind:'default', x:Math.round(window.innerWidth*.06), y:Math.round(window.innerHeight*.04)});
  ...
  if(frame && fallbackUrl){
    frame.addEventListener('error', () => { frame.src = fallbackUrl; }, { once:true });
  }
...
function portfolioFolderItemIconMarkup(){
  return `
    <span class="portfolio-window-folder-icon" aria-hidden="true">
      <svg class="portfolio-window-folder-svg" viewBox="0 0 873.37 694.59" xmlns="http://www.w3.org/2000/svg" focusable="false">
        <path class="portfolio-window-folder-back" d="M827.19,233.59c1.44,7.07-1.22,288.41-.19,302.99-2.63,25.23-22.55,47.9-48.89,47.9,0,0-690.56-.06-690.56-.06-26.3,0-47.97-24.9-47.97-50.41C63.12-105.93-91.47,13.19,440.93,2.38c47.31.47,63.76,60.47,111.23,59.79,0,0,206.75.33,206.75.33,37.96.07,67.96,33.35,68.1,63.6,1,21.77-.56,84.81.17,107.49Z" />
        <path class="portfolio-window-folder-front" d="M827.23,170.69c27.66,5.6,46.58,29.96,46.13,58.18,0,0-.26,412.61-.26,412.61-2.92,27.98-25,53.11-54.21,53.11,0,0-765.69-.06-765.69-.06C24.03,694.53,0,666.92,0,638.64c0,0,.04-516.79.04-516.79,0-24.21,18.2-45.2,39.76-52.11,5.6-1.79,9.73-2.46,15.49-2.45,0,0,246.67.29,246.67.29,42.74-3.02,72.76,50.38,97.4,76.93,12.45,14.79,30.56,25.85,50.87,25.86,0,0,376.99.31,376.99.31Z" />
      </svg>
    </span>
  `;
}
...
function portfolioProjectIconMarkup(item){
  ...
  if(first){
    return `<span class="portfolio-project-thumb" aria-hidden="true"><img src="${first}" alt="" draggable="false" /></span>`;
  }
...
function buildProjectGalleryHTML(project, path=[]){
  const images = (project.assets || []).map((src, index) => `<figure class="portfolio-gallery-card"><img src="${src}" alt="${escapeHTML(project.label)} ${index + 1}" loading="lazy" /></figure>`).join('');
  return `<section class="portfolio-project-view">${buildPortfolioBreadcrumb(path)}<header class="portfolio-project-header"><h2>${escapeHTML(project.label)}</h2></header><div class="portfolio-gallery-grid">${images}</div></section>`;
}

function renderEmptyFolder(){
  return `<div class="portfolio-empty-folder"><p>Esta pasta ainda está vazia.</p></div>`;
}

function renderFolderContent(win, folder, path){
  const files = folder.files || [];
  const content = files.length ? files.map((file, index) => {
    const icon = file.type === 'project' ? portfolioProjectIconMarkup(file) : portfolioFolderItemIconMarkup();
    return `<button class="folder-file portfolio-folder-file ${file.type === 'project' ? 'portfolio-project-file' : 'portfolio-subfolder-file'}" type="button" data-file-index="${index}" title="${escapeHTML(file.label)}">${icon}<small>${escapeHTML(file.label)}</small></button>`;
  }).join('') : renderEmptyFolder();

  const body = win.querySelector('.window-body');
  if(body){
    body.innerHTML = `<div class="portfolio-folder-view">${buildPortfolioBreadcrumb(path)}<div class="folder-grid portfolio-folder-grid">${content}</div></div>`;
  }
...
async function openPortfolioFolderItem(file, currentWin=null, path=[]){
  ...
  createWindow({title:file.label, html, kind:'default', x:Math.round(window.innerWidth*.22), y:Math.round(window.innerHeight*.12)});
}

function openPortfolioProject(project, path=[]){
  const html = buildProjectGalleryHTML(project, path);
  const win = createWindow({title:project.label, html, kind:'default', x:Math.round(window.innerWidth*.12), y:Math.round(window.innerHeight*.08)});
  win.classList.add('portfolio-project-window');
}

function openFolder(folder, existingPath=[]){
  const win = createWindow({title:folder.label, html:`<div class="portfolio-folder-view"></div>`, kind:'folder-window', x:Math.round(window.innerWidth*.24), y:Math.round(window.innerHeight*.12)});
