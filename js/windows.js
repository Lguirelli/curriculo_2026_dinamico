...
function bindPortfolioNavigation(win, path){
...
function renderEmptyFolder(){
  return `
    <div class="portfolio-empty-folder">
      <p>Esta pasta ainda está vazia.</p>
    </div>
  `;
}
...
function renderFolderContent(win, folder, path){
  ...
  if(body){
    body.innerHTML = `
      <div class="portfolio-folder-view">
        ${buildPortfolioBreadcrumb(path)}
        <div class="folder-grid portfolio-folder-grid">${content}</div>
      </div>
    `;
  }
  ...
  bindPortfolioNavigation(win, path);
...
async function renderInlineFileContent(win, file, html, path){
  ...
  if(body){
    body.innerHTML = `
      <section class="portfolio-project-view">
        ${buildPortfolioBreadcrumb(nextPath)}
        <div class="portfolio-inline-file-content">${html}</div>
      </section>
    `;
  }

  const title = win.querySelector('.window-title, [data-window-title]');
  if(title) title.textContent = file.label;

  bindPortfolioNavigation(win, nextPath);
}
...
async function openPortfolioFolderItem(file, currentWin=null, path=[]){
  ...
  if(currentWin){
    await renderInlineFileContent(currentWin, file, html, path);
    return;
  }
